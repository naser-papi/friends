...
export const getUseFriends = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    const {id} = req.params;
    const user = await User.findById(id) as IUserSchema;
    if (!user) next(new Error("invalid user id"));
...
export const getUserPosts = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    const {id} = req.params;
    const user = await User.findById(id) as IUserSchema;
    if (!user) next(new Error("invalid user id"));
...
export const addRemoveFriend = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
