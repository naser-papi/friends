...
server.use((err: IError, req: express.Request, res: express.Response, next: express.NextFunction) => {
    if (err) {
...
mongoose.connect(process.env.DATABASE_URI, {dbName: process.env.DATABASE_NAME}).then(() => {
    server.listen(Number(process.env.PORT), process.env.SERVER, () => {
        console.log(`Server is runing on this address: http://${process.env.SERVER}:${process.env.PORT}`);
    });
